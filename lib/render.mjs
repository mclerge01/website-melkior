...
/**
 * Render trusted content Markdown to sanitized HTML for templates.
 *
...
/**
 * Validate and normalize the admin-configured Fouita Instagram embed.
 *
 * @param {Record<string, unknown>} shared - Shared settings object.
 * @returns {{url: string, title: string}} Safe iframe URL and title.
 */
...
/**
 * Convert configured Markdown fields into trusted sanitized HTML wrappers.
 *
 * @param {Record<string, unknown>} data - Site settings data.
 * @returns {Record<string, unknown>} Deep-cloned and processed settings.
 */
...
/**
 * Render the small project template language against prepared page data.
 *
 * @param {string} template - HTML template containing {{ }} placeholders.
 * @param {Record<string, unknown>} data - Render data.
...
export function render(template, data) {
  ...
  out = out.replace(/\{\{#each\s+([\w.]+)\}\}([\s\S]*?)\{\{\/each\}\}/g, (_, key, body) => {
    ...
    return arr.map((item, index) => {
      let chunk = renderIfBlocks(body.replace(/\{\{@index\}\}/g, String(index)), item);
